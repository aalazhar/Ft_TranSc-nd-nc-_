...
import { ConversationsUpdate, addNewConv, fetchChatData } from "../Slices/chatSlice";
...
export default function chat() {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      dispatch(ConversationsUpdate(selectedConv));
    }, [selectedConv]);
  ...
  const handleSendMessage = (newMessage: string) => {
    if (selectConvId !== null) {
      ...
      prevConversations.map((conversation: any) => {
        if (conversation?.id === selectConvId) {
          return {
            ...conversation,
            timestamp: timestamp,
            senderId: userId,
            ...
            <div className="h-[80%] min-h-[600px] medium:min-h-[700px] m-auto w-[410px] medium:w-[90%] mt-11">
              <div className="w-full h-[90%] xMedium:h-full flex xMedium:flex xMedium:justify-between xMedium:items-center ">
                ...
                { (<div id="id_2" className={`${showConversations ? 'hidden' : 'flex'} flex-col xMedium:block w-full h-full xMedium:w-[65%] bg-[#131313] border-2 border-[#323232] rounded-xl`}>
                  ...
                  <ChatContent conversations={selectedConv} id={(selectedConv.find((conversation) => conversation.id === selectConvId) || { id: 0 }).id} messages={selectedConv.find((conversation) => conversation.id === selectConvId)?.messages || []} conversation={selectedConv.find((conversation) => conversation.id === selectConvId) as Conversation}/>
