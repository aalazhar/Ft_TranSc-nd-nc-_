  ...
  return (
    <div className='flex justify-between min-w-[350px] h-16 Large:h-24 mx-6'>
      <div className='flex w-[50%] text-[#E58E27] items-center justify-around'>
        <h3 className='ml-7 medium:ml-3 text-sm Large:text-xl w-28 whitespace-normal overflow-x-auto scrollbar-none'>{pageName}</h3>
        <div className='relative h-full w-full'>
          <div>
            <input maxLength={9} type='text' name={searchInput} onChange={handleSearch} className={`${hideIt ? "w-[220px]" : "hidden"} bg-red-900 medium:block top-[18px] absolute Large:top-7 text-orange-100 pl-1 py-1 text-sm focus:outline-none Large:w-[400px] w-52 Large:text-lg medium:w-[300px] border rounded-lg bg-transparent border-[#E58E27]`}></input>
            <ul className={` overflow-y-auto scrollbar-hide max-h-12 list-none p-1 m-0 absolute top-[52px] text-sm Large:w-[400px] w-[220px] Large:text-lg medium:w-[300px] ${searchInput && userData.length > 0 ? "" : "hidden"} ${searchInput && userData.length > 0 ? "" : "bg-opacity-0"} border-t-0 rounded-b-lg bg-opacity-80 z-40 bg-[#323232] border-[#E58E27]`}>
