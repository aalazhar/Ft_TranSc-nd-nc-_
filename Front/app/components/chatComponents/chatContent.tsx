...
import { Conversation, ConversationsUpdate, addNewConv, clearConversations, fetchChatData } from '@/app/Slices/chatSlice';
...
function ChatContent({messages, conversation, conversations, id}: ChatContentProps) {
  ...
  useEffect(() => {
    ...
    return (
        <div className="max-h-[80%] h-[78%] px-6 py-1 overflow-y-auto scrollbar-hide">
          { conversations[id]?.messages?.slice(0).reverse().map((message, index) => (
            <div key={index} className={`py-5 flex flex-row w-full ${!message.isOwner ? "justify-end" : "justify-start"}`}>
              <div className={`h-10 w-10 ${!message.isOwner ? "order-2" : "order-1"}`}>
                <img className='h-10 w-10 rounded-full ' src={message.isOwner ? ownerAvatar : message.avatar}></img>
              </div>
